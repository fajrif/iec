(function ($) {
    ...
    $(document).ready(function () {
        ...
        if ($('.full-width-pull-menu').length > 0) {
            $(document).on('click', '.full-width-pull-menu .inner-link', function (e) {
                ...
                var _this = $(this);
								...
								if (hasPos > -1) {
									var target = link.substring(hasPos);
									setTimeout(function () {
											if ($(target).length > 0) {
													$('html, body').stop()
																	.animate({
        ...
        var $business_filter = $('.business-line');
        $business_filter.imagesLoaded(function () {
            $business_filter.removeClass('grid-loading');
        });

        var $gallery_filter = $('.gallery.collection');
        $gallery_filter.imagesLoaded(function () {
            $gallery_filter.removeClass('grid-loading');
        });

        //var $cover_background = $('.cover-background');
        //$cover_background.imagesLoaded( { background: true }, function () {
            //$cover_background.removeClass('grid-loading');
        //});
